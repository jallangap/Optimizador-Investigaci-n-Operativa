...
class RedesModel:
    ...
    def _parse_undirected_weighted(self, aristas: List[str]) -> Tuple[Optional[List[Tuple[str, str, int]]], Optional[str]]:
        ...
        for a in aristas:
            ...
            if len(parts) != 3:
                return None, f"Formato incorrecto: '{a}'. Usa Origen-Destino-Peso"
            u, v, w = parts
            try:
                w_i = int(w)
            except ValueError:
                return None, f"Peso no válido en '{a}'."
    ...
    def _parse_directed_capacity(self, aristas: List[str]) -> Tuple[Optional[List[Tuple[str, str, int]]], Optional[str]]:
        ...
        for a in aristas:
            ...
            parts = [self._clean_token(p) for p in a.split("-")]
            if len(parts) != 3:
                return None, f"Formato incorrecto: '{a}'. Usa Origen-Destino-Capacidad"
            u, v, c = parts
            try:
                c_i = int(c)
            except ValueError:
                return None, f"Capacidad no válida en '{a}'."
    ...
    def _parse_directed_capacity_cost(self, aristas: List[str]) -> Tuple[Optional[List[Tuple[str, str, int, int]]], Optional[str]]:
        ...
        for a in aristas:
            ...
            parts = [self._clean_token(p) for p in a.split("-")]
            if len(parts) != 4:
                return None, f"Formato incorrecto: '{a}'. Usa Origen-Destino-Capacidad-Costo"
            u, v, cap, cost = parts
            try:
                cap_i = int(cap)
                cost_i = int(cost)
            except ValueError:
                return None, f"Valores no válidos en '{a}'."
    ...
    def crear_grafico(
        ...
        if highlight_path and len(highlight_path) >= 2:
            for i in range(len(highlight_path) - 1):
                ...
                highlight_set.add((highlight_path[i + 1], highlight_path[i]))
    ...
    def ruta_mas_corta(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not nodes: return {"error": "No se proporcionaron nodos válidos."}
        s, t = nodes[0], nodes[-1]
        ...
        if s not in adj or t not in adj:
            return {"error": "Nodo origen/destino no existe."}
        ...
        while pq:
            ...
            for v, w in adj.get(u, []):
                if w < 0: return {"error": "Pesos negativos no soportados."}
                nd = d + w
        ...
        all_nodes = list({*nodes, *[u for u, _, _ in edges], *[v for _, v, _ in edges]})
        self.crear_grafico(nodes=all_nodes, edges=edge_list, edge_labels=labels, directed=False, highlight_path=ruta)

        return {"metodo": "Ruta Más Corta", "ruta": ruta, "distancia": dist[t]}
    ...
    def arbol_minima_expansion(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(nodes) > 0 and len(mst) != len(nodes) - 1:
            return {"error": "El grafo no es conexo."}

        edge_list = [(u, v) for (u, v, _) in edges]
        labels = {(u, v): str(w) for (u, v, w) in edges}
        self.crear_grafico(nodes=nodes, edges=edge_list, edge_labels=labels, directed=False)

        return {"metodo": "Árbol de Mínima Expansión", "arbol": [(u, v) for (u, v, _) in mst], "costo_total": total, "arbol_detalle": mst}
    ...
    def flujo_maximo(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        ...
        nodes = [n.strip() for n in datos.get("nodos", []) if n.strip()]
        if not nodes: return {"error": "No hay nodos."}
        s, t = nodes[0], nodes[-1]
    ...
    def flujo_costo_minimo(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        ...
        g = _ResidualGraph()
        original_edges: List[Tuple[str, str, _Edge, int]] = []
        node_set = set(nodes)
        ...
        if s not in g.adj or t not in g.adj: return {"error": "Nodo no existe."}
            ...
            if datos and edges_in:
                ...
                if not err and parsed is not None:
                    ...
                    for (u, v, c) in parsed:
                        ...
                        if c - f > 0: adj.setdefault(u, []).append(v)
                        if f > 0: adj.setdefault(v, []).append(u)
                    ...
                    if s is not None:
                        ...
                        while stack:
                            ...
                            reachable.add(u)
                            for v in adj.get(u, []):
                                if v not in reachable: stack.append(v)
                    ...
                    facts["min_cut"] = {"edges": cut, "capacity": cut_cap}
